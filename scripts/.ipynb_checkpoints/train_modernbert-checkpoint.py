...
def build_parser() -> argparse.ArgumentParser:
    ...
    dataset_group.add_argument(
        "--max-answers-per-source-per-row",
        type=int,
        default=1,
        help=(
            "Maximum number of answers to keep from each source column per row. "
            "Defaults to 1 so each row contributes one human answer and one answer "
            "from each selected AI column."
        ),
    )
    dataset_group.add_argument(
        "--validation-size",
    ...
    dataset_group.add_argument(
        "--save-splits-dir",
        default=None,
        help="Optional path for saving the flattened train/validation DatasetDict.",
    )
...
def sample_answers_for_row(
    ...
    *,
    ...
    if max_answers <= 0:
        raise ValueError("--max-answers-per-source-per-row must be > 0.")
    if len(answers) <= max_answers:
        return answers

    rng = random.Random(f"{seed}:{split_name}:{row_index}:{source_key}")
    sampled_indices = sorted(rng.sample(range(len(answers)), max_answers))
    return [answers[index] for index in sampled_indices]
...
def flatten_hc3_split(split_dataset: Dataset, args: argparse.Namespace, split_name: str) -> Dataset:
    ...
    def explode_batch(batch: dict[str, list[Any]], indices: list[int]) -> dict[str, list[Any]]:
        ...
        for idx_in_batch, row_index in enumerate(indices):
            ...
            human_answers = sample_answers_for_row(
                human_answers,
                max_answers=args.max_answers_per_source_per_row,
                seed=args.seed,
                split_name=split_name,
                row_index=row_index,
                source_key=args.human_answers_column,
            )
            ...
            for ai_column in ai_answer_columns:
                ...
                ai_answers = sample_answers_for_row(
                    ai_answers,
                    max_answers=args.max_answers_per_source_per_row,
                    seed=args.seed,
                    split_name=split_name,
                    row_index=row_index,
                    source_key=ai_column,
                )
...
def split_dataset(flattened_dataset: Dataset, args: argparse.Namespace) -> DatasetDict:
    ...
    dataset_split = flattened_dataset.train_test_split(
